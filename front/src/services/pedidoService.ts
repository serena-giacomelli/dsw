...
export const pedidoService = {
  ...
  async crearPedido(pedidoData: PedidoData, token: string) {
    try {
      console.log('Enviando pedido a:', `http://localhost:3000/api/pedido`);
  ...
  async crearLineaPedido(lineaPedidoData: LineaPedidoData, token: string) {
    try {
      console.log('Enviando línea de pedido a:', `http://localhost:3000/api/lineaPed`);
      console.log('Datos de la línea:', lineaPedidoData);
      
      const response = await fetch(`http://localhost:3000/api/lineaPed`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
  ...
  async crearPago(pagoData: PagoData, token: string) {
    try {
      console.log('Enviando pago a:', `http://localhost:3000/api/pago`);
  ...
  async finalizarCompraCompleta(compraData: CompraCompleta, token: string) {
    try {
      ...
      const response = await fetch(`http://localhost:3000/api/pedido/finalizar`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
          'Authorization': `Bearer ${token}`
