...
async function findByTipoProducto(req: Request, res: Response) {
  try {
    const codtipoProducto = parseInt(req.params.codtipoProducto);
...
async function removeOferta(req: Request, res: Response) {
  try {
    const { id } = req.params;
    const result = await repository.removeOferta({ id });
    res.json(result);
  } catch (error) {
    res.status(500).json({ message: 'Error al eliminar el producto de oferta', error });
  }
}
